...
class MyHelpers:
    ...
    def check_course_instructor(self, course):
        ...
        if not self.is_admin() and not self.is_course_instructor(course, self.user):
            msg = "Permission denied"
            if self.application.debug:
...
class ManageInstructor(MyRequestHandler):
    ...
    def post(self, course_id, instructor_id):
        ...
        if instructor in course.students:
            if not self.is_admin():
                self.json_error(400, "Permission denied" " (cannot modify instructors)")
...
class DownloadAssignment(MyRequestHandler):
    ...
    def get(self, course_id, assignment_id, student_id):
        ...
        self.json_success(files=files, timestamp=self.strftime(submission.timestamp))
...
def main(argv=None):  # pragma: no cover
    ...
    parser.add_argument("--admins", help="admin user ids (comma splitted)", default="")
    parser.add_argument(
