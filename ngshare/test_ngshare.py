...
def server_communication(url, data=None, params=None, method="GET"):
    ...
    if method != "POST":
        ...
        body = urlencode(data)
    response = yield hc.fetch(actual_url, method=method, body=body, raise_error=False)
    return response
...
def test_add_course(http_client, base_url):
    ...
    yield from assert_fail(url + "course3", method="POST", msg="Course already exists")
...
def test_add_instructor(http_client, base_url):
    ...
    yield from assert_success(url + "course2/lawrence", data=data, method="POST")
    assert (
        len((yield from assert_success("/api/instructors/course2"))["instructors"]) == 2
    )
    ...
    user = "root"
    yield from assert_success(url + "course1/lawrence", data=data, method="POST")
    assert (
        len((yield from assert_success("/api/instructors/course1"))["instructors"]) == 2
    )
    assert len((yield from assert_success("/api/students/course1"))["students"]) == 0
    # Test adding non-existing instructor
    data = {"first_name": "", "last_name": "", "email": ""}
    yield from assert_success(url + "course3/instructor", data=data, method="POST")
...
def test_add_student(http_client, base_url):
    ...
    data["email"] = "lawrence_course2_email"
    yield from assert_success(url + "course2/lawrence", data=data, method="POST")
    assert len((yield from assert_success("/api/students/course2"))["students"]) == 2
    ...
    assert len((yield from assert_success("/api/students/course1"))["students"]) == 0
    user = "root"
    yield from assert_success("/api/instructor/course1/lawrence", method="DELETE")
    user = "kevin"
    yield from assert_success(url + "course1/lawrence", data=data, method="POST")
    assert (
        len((yield from assert_success("/api/instructors/course1"))["instructors"]) == 1
    )
    assert len((yield from assert_success("/api/students/course1"))["students"]) == 1
    # Test adding non-existing instructor
    user = "eric"
    data = {"first_name": "", "last_name": "", "email": ""}
...
def test_download_assignment(http_client, base_url):
    ...
    yield from assert_fail(url + "course1/challenger", msg="Assignment not found")
    # Check list_only
    files = (yield from assert_success(url + "course1/challenge?list_only=true"))[
        "files"
    ]
...
def test_release_assignment(http_client, base_url):
    ...
    yield from assert_success(url + "course1/challenger", method="POST", data=data)
    yield from assert_fail(
...
def test_list_submissions(http_client, base_url):
    ...
    yield from assert_fail(url + "course1/challenges", msg="Assignment not found")
...
def test_list_student_submission(http_client, base_url):
    ...
    yield from assert_fail(url + "course1/challenges/st", msg="Assignment not found")
    yield from assert_fail(url + "course1/challenge/st", msg="Student not found")
...
def test_submit_assignment(http_client, base_url):
    ...
    yield from assert_fail(url + "jkl/challenge", method="POST", msg="Course not found")
    yield from assert_fail(
...
def test_download_feedback(http_client, base_url):
    ...
    yield from assert_fail(url + "course1/challenges/st", msg="Assignment not found")
    yield from assert_fail(url + "course1/challenge/st", msg="Student not found")
    meta = yield from assert_success("/api/submission/course1/challenge/lawrence")
...
def test_remove_course(http_client, base_url):
    ...
    yield from assert_success(url + "course3", method="DELETE")
    yield from assert_fail(url + "course4", method="DELETE", msg="Course not found")
    resp = yield from assert_success(
...
def test_nodebug(http_client, base_url):
    ...
    hc, bu = http_client, base_url
    user = "none"
    yield from assert_fail(url, params={"action": "clear"}, msg="Debug mode is off")
    yield from assert_fail(url, params={"action": "init"}, msg="Debug mode is off")
    response = yield from server_communication("/api/random-page")
    assert response.code == 404
